<template>
  <div class="teacher-certificate">
    ...
    <div class="main-wrap">
      ...
      <div class="btn-group">
        <el-button :disabled="form.currPage == 1" @click="search(0)">上一页</el-button>
        <el-button @click="search(1)" :disabled="form.currPage === maxPage">下一页</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async asyncData({ app, params }) {
    let form = {
      currPage: params.id ? Number(params.id) : 1,
      pageSize: 6
    }
    let list = []
    let maxPage = 0
    try {
      const { resultData, totalCount } = await app.$api.showIndexTeacher(form)
      list = resultData
  ...
  methods: {
    search(type) {
      // type 0上一页 1下一页
      if (type === 0) {
        this.form.currPage = this.form.currPage - 1
      } else if (type === 1) {
        this.form.currPage = this.form.currPage + 1
      }
      this.$router.push({ path: `/teacher-certificate/${this.form.currPage}` })
    }
  }
}
</script>
